...
select set_config('request.jwt.claims','{"sub":"10000000-0000-0000-0000-000000001000","role":"authenticated"}',true);
...
select is((select count(*) from public.submissions),999::bigint,'999 submissions before final slot');
select lives_ok($$select public.submit_ranking('Last voter',(select rankings from capacity_fixture))$$,'Submission 1000 is accepted');
select is((select count(*) from public.submissions),1000::bigint,'Exactly 1000 submissions stored');
reset role;
select set_config('request.jwt.claims','{"sub":"10000000-0000-0000-0000-000000001001","role":"authenticated"}',true);
set local role authenticated;
select throws_ok($$select public.submit_ranking('Too late',(select rankings from capacity_fixture))$$,'PT429',null,'Submission 1001 is rejected');
reset role;
...
select is((select count(*) from public.submissions),1000::bigint,'Updating does not consume capacity');
