-- The original migration has been applied remotely; extend it with a new migration.
...
begin
  ...
  if not exists (select 1 from public.submissions where user_id = actor)
     and (select count(*) from public.submissions) >= 1000 then
    raise exception 'This tier list has reached its limit of 1,000 submissions. Existing voters can still update their ranking.'
      using errcode = 'PT429';
  end if;
